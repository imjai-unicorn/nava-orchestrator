...
"""
Quality Validator Core - Fixed Schema
...
async def batch_quality_validation(batch_request: BatchQualityRequest):
    """Validate multiple responses in batch"""
    
    try:
        results = []
        
        for i, request in enumerate(batch_request.responses[:20]):  # Limit to 20 responses
            validation = await quality_validator.validate_quality(request)
            
            results.append({
                "index": i,
                "response_text": request.response_text[:100] + "..." if len(request.response_text) > 100 else request.response_text,
                "overall_score": validation.get("overall_score", 0.5),
                "quality_level": validation.get("quality_level", "acceptable"),
                "passes_threshold": validation.get("overall_score", 0.5) >= batch_request.threshold,
...
@quality_router.post("/batch/simple")
async def simple_batch_quality_validation(responses: List[str], threshold: float = 0.75):
    """Validate multiple text responses (simple version)"""
    ...
    try:
        ...
        logger.error(f"❌ Simple batch quality validation error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def simple_batch_quality_validation(responses: List[str], threshold: float = 0.75):
    ...
    try:
        ...
        for i, response_text in enumerate(responses[:20]):
            request = QualityRequest(response_text=response_text)
            validation = await quality_validator.validate_quality(request)
